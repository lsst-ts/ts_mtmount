...
* `MTMountCsc`:
    ...
    * Update config schema to v3.
      Eliminate ``camera_cable_wrap_interval``.
      Add ``ack_timeout_track_target``.
    ...
    * Go to fault if a ``trackTarget`` command times out, and potentially use a new config timeout parameter
      (config.ack_timeout_track_target instead of config.ack_timeout) which will probably be smaller.
...
* `MTMountCommander`:
    ...
    * Hide more unwanted data, to avoid overwhelming the display, including:
      ...
      * Only publish interesting fields of logMessage.
...
* Simplify the code for low-level commands (cleanup that has been pending since the low-level controller started issuing replies as yaml):
    ...
    * Change commands to seet ``sequence_id`` and ``timestamp`` fields to zero, instead of guessing correct values.
